...
| - Agent/MCP  |                    | - Agent context |                            | - Context sync  |
|             |                    | - HTML 検索      |                            | - Search       |
|             |                    |                  |                            | - Status bar   |
+-------------+                    +------------------+                            +----------------+
```
...
| エントリの削除 | ユーザーの明示操作のみ。個別削除、全削除、フォルダーと子孫に含まれるエントリの一括削除を用意する。セッションから外すだけで、ディスク上のファイルとフォルダーは削除しない |
| ブラウザーからのHTML追加 | macOSでネイティブのファイル選択とフォルダー選択を提供する。サーバー識別子を検証し、選択したHTMLの絶対パスをセッションへ追加する |
| フォルダーからの追加 | 直下のみ、または子孫を含むHTMLを決定的な相対パス順で追加する。シンボリックリンクはたどらず、ディスク上の項目を変更しない |
...
| 組み込みドキュメント | セッションとは独立したメイン表示として英語版と日本語版を同梱する。ページと言語は URL から復元する |
| AIエージェント連携 | CLIとstdio MCPから、選択中および表示順の全ファイルについてタイトルと絶対パスを読み取り専用で提供する。タイトルは候補選定用の信頼できないラベルとして、空白を統一し、Unicode制御文字・書式制御文字を削除し、200コードポイントへ制限する。パスは識別の正本とする。現在のメイン表示は一括コンテキストに含め、HTML本文は返さず、クライアント固有の設定変更は行わない |
| 配色テーマ | 意味的な色トークンを使い、OS の配色設定に応じてライトテーマとダークテーマを切り替える |
...
利用方法はアプリ内のドキュメントとして配布物へ含める。
英語を既定とし、同じ内容の日本語版へ切り替えられる構成にする。
ドキュメントは通常のセッションエントリから分離し、ファイルの選択状態を変更しない。
ページと言語を URL に保持し、再読み込みとブラウザー履歴から表示を復元する。
