...
| AIエージェント連携 | CLIとstdio MCPから、選択中および表示順の全ファイルについてタイトルと絶対パスを読み取り専用で提供する。タイトルは候補選定用の信頼できないラベル、パスは識別の正本とする。現在のメイン表示は一括コンテキストに含め、HTML本文は返さず、クライアント固有の設定変更は行わない |
